            ...
            <div className="mb-6 md:mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4 md:gap-6 px-4 md:px-0">
                <div>
                    <h1 className="text-3xl md:text-4xl font-black text-foreground tracking-tight mb-1">
                        Task <span className="text-brand-accent italic">Board</span>
                    </h1>
                ...
                <div className="flex flex-wrap items-center gap-2 md:gap-3">
                    <div className="relative group flex-1 md:flex-none">
                        <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-text-muted group-focus-within:text-brand-accent transition-colors" size={16} />
                        <input 
                            type="text" 
                            placeholder="Search tasks..." 
                            value={searchQuery}
                            onChange={(e) => setSearchQuery(e.target.value)}
                            className="bg-accent-slate border border-card-border rounded-xl md:rounded-2xl py-2.5 md:py-3 pl-10 md:pl-12 pr-4 md:pr-6 text-xs md:text-sm font-bold text-foreground focus:outline-none focus:ring-2 focus:ring-brand-accent/20 focus:border-brand-accent transition-all w-full md:w-64 shadow-sm"
                        />
                    </div>
                    <button 
                        onClick={() => setIsSettingsOpen(true)}
                        className="p-2.5 md:p-3 bg-accent-slate border border-card-border rounded-xl md:rounded-2xl text-text-muted hover:text-brand-accent hover:border-brand-accent/50 transition-all shadow-sm active:scale-90"
                    >
                        <Settings size={18} />
                    </button>
                    <button 
                        onClick={() => handleAddTask()}
                        className="flex items-center justify-center gap-2 bg-brand-accent text-background px-4 md:px-6 py-2.5 md:py-3 rounded-xl md:rounded-2xl font-black text-[10px] md:text-xs uppercase tracking-widest shadow-lg shadow-brand-accent/20 hover:scale-[1.02] active:scale-95 transition-all flex-1 md:flex-none"
                    >
                        <Plus size={16} />

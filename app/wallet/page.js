...
export default function Wallet() {
    const { data, loading, addRecord, totalIncome, totalExpenses, netProfit } = useAppContext();
                ...
                <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {data.transactions.length === 0 ? (
                            <tr>
                                <td colSpan={4} className="text-center p-8 text-slate-400">No transactions yet</td>
                            </tr>
                        ) : (
                            data.transactions.map(tx => (
                                <tr key={tx.id} className="hover:bg-slate-50">
                                    ...
                                    <td className={`p-4 text-right font-bold ${tx.type === 'income' ? 'text-emerald-600' : 'text-red-500'}`}>
